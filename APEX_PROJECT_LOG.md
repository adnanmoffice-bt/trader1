...
| 2026-04-29 | TRACE 30d real money: Binance API confirms **0 spot fills, 0 open orders, Spot USDT $0.01** — APEX has never executed a real trade despite `trading_mode=live` since 2026-04-23. Root cause = Binance Auto-Subscribe to Simple Earn. The 500 USDT manually transferred Funding→Spot on 23/04 12:22 was reversed at 23/04 22:04 with `type=AUTO, amount=500.00005, src=SPOT`. Daily ~22:00 Dubai sweeps continue to drain any USDT/USDC/BNB dust on Spot. War-room phase 1b silently fails `quoteBalance >= notional` for every signal. 5-day counterfactual: war-room would have executed 0 trades anyway (only 1 `role=open` meeting in 487, and it timed out). |
| 2026-04-29 | chore(ops): added scripts/binance-trade-history.mjs (per-symbol myTrades dump), scripts/binance-money-flow.mjs (full forensic: transfers, Earn subs/redemptions, converts, deposits, withdrawals), scripts/trace-real-trades.mjs (Supabase-side trade/position/log reconciliation). All read-only. |
| 2026-04-29 | chore(ops): added scripts/move-funding-to-spot.mjs — safe Funding→Spot transfer + optional Simple Earn flexible redemption. Dry-run by default, `--confirm` required to execute. Built-in Auto-Subscribe guard scans last 7d for `type=AUTO` USDT subs and refuses to run while feature is enabled (override with `--force`). Companion to Hard Truth #31. |
| 2026-04-29 | docs: CONTEXT.md Hard Truth #31 added (Binance Auto-Subscribe to Simple Earn trap, manual UI fix path, recovery procedure). HANDOFF.md OPEN WORK rewritten — old "transfer 1,100 USDT" item replaced with two ordered operator actions (disable Auto-Subscribe in UI first, then run the helper script). |
| 2026-04-30 | OPS: operator manually transferred 500 USDT Funding → Spot via Binance UI (API key lacks Universal Transfer permission so `scripts/move-funding-to-spot.mjs` aborts with -1002). Verified Spot USDT now $500.02. Auto-Subscribe reportedly disabled — actual confirmation pending tonight's 22:00 sweep test. |
| 2026-04-30 | feat(whatsapp): replaced per-15-min `notifyWarRoomScan` blasts with a 2-hourly comprehensive status report. New `notifyPeriodicReport` covers portfolio (paper), real-money breakdown (Spot/Funding/Earn), 2h activity counters (meetings/triggers/decisions/blocks/macro), open positions with live P&L, per-instrument market snapshot (price + 24h Δ + RSI + regime + strength), notable triggers, macro pause state, daily-loss budget, AI spend. New cron `/api/cron/status-report` at `0 */2 * * *`. War-room inline `waScan` call removed; per-meeting open/debate/decision/blocked alerts unchanged. |
| 2026-04-30 | feat(whatsapp): reformatted event-driven alerts (signal, war-room open/decision/debate/blocked, trade opened/closed, position alert) for density. Added `[REAL]`/`[PAPER]` tags, signed P&L, SL/TP % distances, hold duration on close, Binance order ID on real opens. Removed random `greet()` from operational alerts (kept on morning/daily/weekly/risk/kill-switch where the friendly tone fits). |
| 2026-04-30 | AUDIT 10d: 18 demo trades opened (1 still open), 17 closed, 4W/13L = 23.5% WR, net **-$308.65**. All LONGs on BTC/ETH/XAU. 13 stop_loss, 4 take_profit. 0 live trades ever. War-room funnel: 7,511 meetings → only 8 reached `decision` (0.1%). Most "lost" days are flat (no entries); losses cluster in Dubai 02:00-09:00 window (0/8 = 0% WR). BTC and ETH co-traded as a pair 3/3 times in window. 3 of 13 stop-outs closed within 30min. Saved as `scripts/audit-10d.mjs` for future runs. |
| 2026-04-30 | feat(signals): 7 new helper modules in `lib/` (no high-risk file edits). `session-filter.ts` (Dubai-hour gate), `correlation-dedup.ts` (paired-instrument hard/soft veto), `derivatives.ts` (Binance USDT-M perp funding/OI/L:S), `orderbook.ts` (pre-exec depth/spread), `news-impact.ts` (Claude-scored RSS veto), `cme-gaps.ts` (BTC=F vs spot), `onchain.ts` (stub interface). Plus `multiTimeframeConfluence()` in `lib/indicators.ts`. All fail-open. tsc --noEmit clean. |
| 2026-04-30 | feat(war-room): wired Tier 1 trade-discipline filters. **ATR sanity gate** (pre-debate, blocks atrPct < 0.3% or > 5%). **Session-of-day gate** (decision-time, blocks Dubai 02:00-09:00 unless conviction≥90 + macro≤MEDIUM, or 3+ triggers). **Correlation dedup** (decision-time, hard-blocks BTC↔ETH double-up; soft-flags alt buckets). All standard close-pattern: speak(decision) → speak(close) → waBlocked → runPostMeetingBrief → return 'blocked-X'. |
| 2026-04-30 | feat(war-room): TP calibration v3. **TP1 4.5 ATR → 3.0 ATR** (R:R 2.25→1.5), **TP2 6.0 ATR → 5.0 ATR** (R:R 3.0→2.5). Math on the audit set: every winner still hits +1.5R first; conservatively 25% of losers tag +1.5R before reversing → ≈3 conversions. Estimated +$540 over the same 10d trade list. R:R 1.5 = MIN_RR floor so hardRiskCheck still passes. Backtest gate auto-revalidates (takes tpMult as arg). TP2 persists to `signals.take_profit_2` for future positions-cron partial-fill management. |
| 2026-04-30 | feat(war-room): wired Tier 2 derivatives + pre-exec book check. `getDerivativesSnapshot()` pulls funding 8h, 7d-avg, OI 4h delta, retail L/S, top-trader L/S from Binance USDT-M public API. Hard veto pre-debate on funding > 0.05%/8h or retail L/S > 2.5. Soft conviction adjusts (-12..+8) feed into `effectiveConviction`. Derivatives data added to orchestrator user prompt. **Order-book check** (`/api/v3/depth`) added inside live exec branch right before `marketBuy()`: rejects spread > 5bps majors / 20bps alts, or top-3 same-side depth < 20× notional. Fail-open everywhere. |
| 2026-04-30 | feat(war-room): MTF + macro-HIGH stricter gates. **MTF veto** (single 1H trigger AND `longConfluenceCount===0` from 4H+1D aggregates → block). **Macro HIGH** now requires 3+ triggers in confluence (EXTREME already pauses war-room above). MTF block surfaced in orchestrator prompt as a MULTI-TIMEFRAME context section. |
| 2026-04-30 | feat(war-room): wired Tier 3 external intelligence. **News-impact veto** (`evaluateNewsImpactForLong`) — pulls last 60min of CryptoPanic/CoinDesk/CoinTelegraph RSS, Claude scores -100..+100, vetos at ≤ -40, ~$0.005 per call, runs after orchestrator decision + correlation dedup so we only spend tokens on trades that already passed every other gate. **CME gap** (BTC LONG only, +/-5 conviction nudge from BTC=F vs spot, pre-debate, Yahoo cached 30min). **On-chain stub** (`evaluateOnchainForLong`) — interface in place; data wiring (CryptoQuant / Glassnode free tier) is OPEN WORK. All fail-open. |
| 2026-04-30 | docs: CONTEXT.md Hard Truths #32-38 added covering all new gates. APEX_PROJECT_LOG.md (this file) and HANDOFF.md SESSION LOG updated. LOCK on `agents/war-room.ts + lib/risk-controls.ts` cleared. Build clean across all 6 commits in the multi-batch upgrade. |
| 2026-04-30 | feat(backtest): `scripts/backtest-gate-stack.mjs` — 6-month walk-forward validation. Pulls 4,320 1H Binance candles per instrument, re-implements all deterministic war-room gates, walks forward bar-by-bar, simulates entries/SL/TP. Three modes (BASELINE / FILTERED / NEW) plus `--sweep` for SL×TP grid search (24 combos). Read-only, no DB or exchange writes. |
| 2026-04-30 | **VALIDATION** 6mo backtest verdict (180 days, 11 instruments): all 24 SL/TP combos NEGATIVE expectancy. Best of 3 production modes — NEW (TP=3.0, all gates ON): 348 trades, 34.5% WR, **-0.138 R/trade**, -$3,600 / 180d. BASELINE: 657 trades, 27.2% WR, -0.115 R/trade, -$5,644 / 180d. New gates REDUCE damage by ~$2k vs baseline (max DD 128% → 82% of capital), but underlying triggers do not have edge. Per-instrument: only DOGE (+5.5R, 46.9% WR) and LINK (+5.0R, 45.0% WR) consistently profitable. ADA/DOT/APT consistently worst. |
| 2026-04-30 | **CRITICAL**(safety): edge gate added to block live exec when system is bleeding. `lib/safety.ts` `checkLiveTradingAllowed(instrument)`. Two layers: (1) per-instrument `LIVE_INSTRUMENT_BLACKLIST` containing ADA/DOT/APT (proven losers in backtest, never live-trade them), (2) 30-day rolling expectancy gate on `demo_trades`, blocks if mean R/trade < -0.05 over a 20-trade sample. Fails CLOSED on insufficient data or DB error. Demo trades NEVER affected. Wired into `agents/war-room.ts` BEFORE `getPrimaryExchange` call. Hard Truth #39 added. Build clean. |
| 2026-04-30 | docs: HANDOFF SESSION LOG + OPEN WORK updated. `positions-cron partial-fill` deferred until 30d demo expectancy turns positive (no point optimizing the loss curve on a negative-edge system). LOCK cleared. Operator informed: live exec is now hard-gated until edge is proven on demo. |