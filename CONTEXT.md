...
43. **CHECK constraint must include every transient state used by code (2026-05-14)** — `external_signals.execution_status` had `('pending','executed','skipped','failed','disabled')` but `claimSignalRow()` writes `'executing'` as the atomic-claim transient state. Postgres rejected every claim with `23514`; supabase-js returned `{data:null, error:{...}}`; the caller only destructured `data`, treating null as "another caller already claimed" → looked like normal race-loss in `agent_logs`. Net effect: external-signal executor was silently dead from 2026-05-13 12:59 → 2026-05-14 08:55 (~13h). Two real Signal Feed XAU/USD signals never reached IG. **Lesson**: (a) CHECK lists must contain EVERY value the application writes, including transient states; (b) NEVER swallow a Supabase error in a critical-path write — always destructure both `data` and `error` and log `error` to `agent_logs`; (c) self-audit cron now alarms on any `execution_status='pending' AND created_at < now()-10min`. Migration: `supabase/migrations/2026-05-14-fix-execution-status-check.sql`. Code fix: `lib/telegram-executor.ts → claimSignalRow()`.
